...
connection_pass=$(cat ./password | awk '{print $1}')
        ...
        read filename
	#echo "please type the remore filename"
	#read filename_local

#filename_local=filename_rmt
...
then
        gnome-terminal --working-directory=/home/eldar/script -x bash -c "./scp_download.exp $connection_user $connection_pass $hostname '$filename'; bash"
else
        ./scp_download.exp $connection_user $connection_pass $hostname "$filename"
fi
