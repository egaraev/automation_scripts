...
then
        gnome-terminal --working-directory=/home/eldar/script -x bash -c "./TASKS/scmcheck.exp $connection_user $connection_pass $hostname; bash"
	#tmux ./runcommand.exp $connection_user $connection_pass $hostname '$command'; bash
else
        ./TASKS/scmcheck.exp $connection_user $connection_pass $hostname
fi
